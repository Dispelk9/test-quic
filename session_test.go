...
// 		// XXX (QDC): adapted to multiple paths
// 		err := sess.handleFrames([]wire.Frame{&wire.PingFrame{}}, sess.paths[0], time.Now())
// 		Expect(err).NotTo(HaveOccurred())
// 	})

// 	It("handles BLOCKED frames", func() {
// 		// XXX (QDC): adapted to multiple paths
// 		err := sess.handleFrames([]wire.Frame{&wire.BlockedFrame{}}, sess.paths[0], time.Now())
// 		Expect(err).NotTo(HaveOccurred())
// 	})

// 	It("errors on GOAWAY frames", func() {
// 		// XXX (QDC): adapted to multiple paths
// 		err := sess.handleFrames([]wire.Frame{&wire.GoawayFrame{}}, sess.paths[0], time.Now())
...
// 		// XXX (QDC): adapted to multiple paths
// 		err := sess.handleFrames([]wire.Frame{&wire.ConnectionCloseFrame{ErrorCode: 42, ReasonPhrase: "foobar"}}, sess.paths[0], time.Now())
// 		Expect(err).NotTo(HaveOccurred())
